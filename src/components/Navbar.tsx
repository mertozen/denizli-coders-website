  ...
  const Navbar: React.FC<NavbarProps> = ({logo, about_tag,events_tag, team_tag, join_button, tg_url}) => {
    ...
    return(
      ...
      <div className="flex flex-col md:flex-row items-center gap-x-6 gap-y-3 md:gap-y-0 text-lg font-bold text-blue-700">
        <a href="#about" className="hover:text-blue-500 transition">{about_tag}</a>
        <a href="#events" className="hover:text-blue-500 transition">{events_tag}</a>
        <a href="#team" className="hover:text-blue-500 transition">{team_tag}</a>
      </div>
      </div>
